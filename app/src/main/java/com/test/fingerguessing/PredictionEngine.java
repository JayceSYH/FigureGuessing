...
{
	final int ADDITIVE_SMOOTHING_FACTOR = 5;
	final int ADDITIVE_SMOOTHING_CONST = 1;
	...
	String dataString;	/** Handle to file holding the game data */

	static private PredictionEngine engine;

	static public PredictionEngine getInstance() {
		return engine;
	}

	static public void initEngine(String inputString) {
		if (!inited) {
			engine = new PredictionEngine(inputString);
			inited = true;
		}
	}
	 ...
	 * holding the data
	 * @param  inputString	- handle to File with data
	 */
	private PredictionEngine(String inputString)
	{
